...
def calculate_accuracy_and_loss(batch_set, dropout, is_training):
    ...
    if use_warpctc:
        total_loss = tf.contrib.warpctc.warp_ctc_loss(logits, batch_y, output_lengths)
    else:
        total_loss = ctc_ops.ctc_loss(logits, batch_y, output_lengths)
...
def get_tower_results(batch_set, optimizer=None, is_training=False):
...
def create_execution_context(set_name):
    ...
    with graph.as_default():
        ...
        tower_results = get_tower_results(data_set, optimizer=optimizer, is_training=is_train)
...
def start_execution_context(execution_context, model_path=None):
    ...
    with graph.as_default():
        ...
        execution_context[1].start_queue_threads(session)
...
def stop_execution_context(execution_context, session, checkpoint_path=None, global_step=None):
...
def calculate_loss_and_report(execution_context, session, epoch=-1, query_report=False):
    ...
    if do_training:
        params.append([apply_gradient_op, merged])
    ...
    if query_report:
        ...
        params.append([results_params, avg_accuracy])
    ...
    for batch in range(int(batches_per_device)):
        ...
        if query_report:
            ...
            collect_results(report_results, result[offset][0])
            # Add batch to total_accuracy
            total_accuracy += result[offset][1]
...
def run_set(set_name, model_path=None, query_report=False):
    ...
    session = start_execution_context(execution_context, model_path=model_path)

    # Applying the batches
    batch_set_result = calculate_loss_and_report(execution_context, session, query_report=query_report)

    # Cleaning up
    stop_execution_context(execution_context, session)
...
def train():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch == 0 or hibernation_path is not None:
            ...
            session = start_execution_context(train_context, hibernation_path)
        ...
        if is_validation_step:
            ...
            hibernation_path = stop_execution_context( \
                train_context, session, checkpoint_path=checkpoint_path, global_step=epoch)
